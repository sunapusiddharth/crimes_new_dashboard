...
import { faFacebook,faTwitter,faInstagram } from "@fortawesome/free-brands-svg-icons"
 ...
 class CrimePeopleLaw extends Component {
    constructor(props) {
        super(props)
        this.state = {
                            ...
                            if (index >= this.state.index && index < (this.state.limit+this.state.index)) {
                                return <Card>
                                    ...
                                    {/* <Link exact={true} to={`/news/single/${image._id}`}> */}
                                        ...
                                        <Card.Text>
                                            <br/>
                                            {`Is a judge from ${image.address}. Has handled ${image.cases.length} cases. Previously worked in ${image.departments} departments . He was recently employed in ${image.departments[0].name} as a ${image.departments[0].position} for a duration of ${image.duration} years. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.`}
                                        </Card.Text>
                                        {/* </Link> */}
                                    </Card.Body>
                                    <Card.Footer>
...
export default withRouter(CrimePeopleLaw)
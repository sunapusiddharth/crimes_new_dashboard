...
import {withRouter} from 'react-router-dom';
 ...
 class CrimePeopleVcitims extends Component {
    constructor(props) {
        super(props)
        this.state = {
                        ...
                        {this.state.images.map((image, index) => {
                            if (index >= this.state.index && index < (this.state.limit+this.state.index)) {
                                return <Card>
                                    ...
                                    <Card.Body>
                                    {/* <Link  to={`/news/single/${image._id}`}> */}
                                        <Card.Title> {image.name}</Card.Title>
                                        <Card.Text>
                                            <br/>
                                            {image.address && `Lived in ${image.address}`}{image.employment && `Worked at ${image.employment[0].company_name} as a ${image.employment[0].employment_type} for a duration of ${image.duration} years. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.`}
                                        </Card.Text>
                                        {/* </Link> */}
                                    </Card.Body>
                                    <Card.Footer>
...
export default withRouter(CrimePeopleVcitims)
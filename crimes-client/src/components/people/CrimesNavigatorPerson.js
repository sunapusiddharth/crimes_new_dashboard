...
import '../../styles/crimes_navigator.css'
import faker from 'faker'

 class CrimesNavigatorPerson extends Component {
    constructor(props) {
        super(props)
        this.state = {
                    ...
                    <CardDeck>
                        {this.state.images.map((image, index) => {
                            if (index >= this.state.index && index < (this.state.limit+this.state.index)) {
                                // let date = new Date(image.occurence_on_date)
                                let date = new Date()
                                let date_string = `${date.getFullYear()}-${date.getMonth()}-${date.getDate()}`
                                return <Card onClick={()=>this.props.onSummaryContentChange(index)}>
                                    <Card.Img variant="top" src={image.imageUrl} />
                                    <Card.Body>
                                    {/* <Link exact={true} to={`/crimes/single/${image._id}`}> */}
                                        {/* <Card.Title> {image.title}</Card.Title> */}
                                        <Card.Title> crime title {image}</Card.Title>
                                        <Card.Text>
                                            {/* <p>Occured at- {`${image.address}`}</p> */}
                                            <p>Occured at-address of crime</p>
                                        to be updated from backend the field {faker.lorem.sentences(3)}</Card.Text>
                                        <Link exact={true} to={`/crimes/single/#`}><Badge variant="dark">Show More</Badge></Link>
                                    </Card.Body>
                                    <Card.Footer>
                                        <small className="text-muted">{date_string}</small>
                                        {/* <Badge variant="dark" className="pull-right crimes_navigator_badge">{image.category}</Badge> */}
                                        <Badge variant="dark" className="pull-right crimes_navigator_badge">image.category</Badge>
                                    </Card.Footer>
                                </Card>
                            }
...
export default withRouter(CrimesNavigatorPerson)
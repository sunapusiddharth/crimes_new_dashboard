...
import SearchResultPersonCard from '../search/SearchResultPersonCard'
...
class SearchPeople extends Component {
    ...
    componentDidMount() {
        ...
        this.props.dispatch(searchPeople("", {}, 0,10))
    }
    ...
    onPageChanged = data => {
        ...
        this.props.dispatch(searchPeople(this.state.search_query, {}, (from-10), pageLimit))
        this.setState({ currentPage, totalPages, pageLimit })
    }

    render() {
        if (this.props.search_loading) {
            return <PageLoader loading={this.props.search_loading} />
        } else {
            if (!Object.keys(this.props.search_results).length) {
                return <Fragment>
                    <div className="row search_people" ref={this.ref}>
                        <div className="col-lg-12 col-md-12 ">
                            <DepartmentSearchBox searchAction={searchPeople} sidhu={"sasasa"} changeSearchQuery={this.changeSearchQuery} />
                        </div>
                ...
                const totalCountries = this.props.total_people;
                ...
                return (
                    <Fragment>
                        <div className="row search_people" ref={this.ref}>
                            ...
                            <DepartmentSearchBox changeSearchQuery={this.changeSearchQuery}  searchAction={searchPeople} sidhu={"search_people"}/>
                            </div>
                        ...
                        {currentCountries.map(result => <SearchResultPersonCard key={result._id} result={result} />)}
                        <div className="container mb-5">
                            <div className="row d-flex flex-row py-5">
                                <div className="w-100 px-4 py-5 d-flex flex-row flex-wrap align-items-center justify-content-between">
                                    <div className="d-flex flex-row align-items-center">
                                        <h2 className={headerClass}>
                                            <strong className="text-secondary">{totalCountries}</strong> People
                                    </h2>
                                    </div>
                                    <div className="d-flex flex-row py-4 align-items-center">
                                        <Pagination totalRecords={this.props.total_people} pageLimit={10} pageNeighbours={1} onPageChanged={this.onPageChanged} />
                                    </div>
                                </div>
                            </div>
                        </div>
...
const mapStateToProps = (state) => ({
    search_results: state.searchReducer.people,
    loading: state.searchReducer.search_loading,
    total_people: state.searchReducer.total_people,
})

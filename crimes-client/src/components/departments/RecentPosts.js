...
export default function RecentPosts(props) {
    ...
    if (posts.length) {
        return (
            <Fragment>
                <h4>Recent Posts <p className="w3-button w3-black w3-right w3-small"><Link to={`/department/${department_id}/recent_posts`} style={{color:"white"}}>Show All</Link></p></h4>
                <Carousel activeIndex={index} direction={direction} onSelect={handleSelect}>
                    {posts.map(post => {
                        // return <SinglePost data={post} />
                        let { attachments, body, changed, image, component, teaser, title, uuid } = post
                        var d = new Date(changed * 1000);
                        let dateString = `${d.getMonth()} ${d.getDay()},${d.getFullYear()}`
                        body = strip(body)
                        let summary = teaser.length ? teaser : body.slice(0, 400)
                        return (
                            <Carousel.Item key={uuid}>
                                <div className="w3-center">
                                    <h5>{component && component.length && component[0].name}</h5>
